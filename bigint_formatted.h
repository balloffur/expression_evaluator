...
struct bigint {
    ...
    // Move semantics optimized operators
    ...
    bigint &operator<<=(int n) {
        while (n > 29) {
            long long carry = 0;
            for (int i = 0; i < digits.size(); i++) {
                long long temp = powers_of_2[29] * digits[i];
        ...
        long long carry = 0;
        for (int i = 0; i < digits.size(); i++) {
            long long temp = powers_of_2[n] * digits[i];
            digits[i] = temp % BASE + carry;
            carry = temp / BASE;
        }
        if (carry) {
    ...
    int shift_to_odd() {
        int ans = 0;
        while (even()) {
            int temp = possible_shifts_to_odd();
            *this >>= temp;
            ans += temp;
        }
        return ans;
        ...
        for (int j = n / 2 - 1; j >= 0; j--) {
            for (;; --q) {
                bigint r1 = (r - (res * 2 * bigint(BASE) + q) * q) * bigint(BASE) * bigint(BASE) +
                            (j > 0 ? (long long)a.digits[2 * j - 1] * BASE + a.digits[2 * j - 2] : 0);
                if (r1 >= 0) {
                    r = r1;
                    break;
                }
...
private:
    ...
    static int compare_abs(const bigint &x, const bigint &y) {
    ...
    bigint mul_karatsuba(const bigint &v) const {
        ...
        std::vector<long long> x(x6.begin(), x6.end());
        std::vector<long long> y(y6.begin(), y6.end());
        ...
        std::vector<long long> c = karatsuba_multiply(x, y);
        bigint res;
        res.sign = sign * v.sign;
        long long carry = 0;
    ...
    static std::vector<long long> karatsuba_multiply(const std::vector<long long> &a,
