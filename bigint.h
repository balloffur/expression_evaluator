...
static const long long powers_of_2[30] = {
    1,        2,        4,        8,         16,        32,
...
// lg(a)>FFT_BORDER || lg(b)>FFT_BORDER. To avoid loss of precision from fft for
// very large numbers --> switch to karatsuba. Probably rather use gmp.
...
struct bigint {
  ...
  void __internal_add(const bigint &v) {
    ...
    for (int i = 0, carry = 0;
         i < (int)std::max(digits.size(), v.digits.size()) || carry; ++i) {
  ...
  friend std::pair<bigint, bigint> divmod(const bigint &a1, const bigint &b1) {
    ...
    for (int i = a.digits.size() - 1; i >= 0; i--) {
      ...
      r += a.digits[i];
      long long s1 =
          r.digits.size() <= b.digits.size() ? 0 : r.digits[b.digits.size()];
      long long s2 = r.digits.size() <= b.digits.size() - 1
                         ? 0
  ...
  int operator%(int v) const {
    if (v == 0) {
      throw std::runtime_error("Division by thero");
    }
    if (v < 0)
      v = -v;
    // optimise for pows of 2 and 5
    if (BASE % v == 0) {
      return (digits.size() == 0 ? 0 : (digits[0] % v) * sign);
    }
    int m = 0;
  ...
  bigint &operator%=(int v) {
    ...
    // optimise for pows of 2 and 5
    if (BASE % v == 0) {
      m = digits.size() == 0 ? 0 : (digits[0] % v) * sign;
    } else {
      for (int i = digits.size() - 1; i >= 0; --i)
        m = (digits[i] + m * (long long)BASE) % v;
    }
    this->digits.resize(1);
    this->digits[0] = m * sign;
    return *this;
  }
  ...
  void operator*=(int v) {
    ...
    for (int i = 0, carry = 0; i < (int)digits.size() || carry; ++i) {
      ...
      // asm("divl %%ecx" : "=digits"(carry), "=d"(digits[i]) : "A"(cur),
      // "c"(base));
      /*
       int val;
       __asm {
  ...
  void multiply_fft(const std::vector<int> &x, const std::vector<int> &y,
      ...
      if (digits[i])
        for (int j = 0, carry = 0; j < (int)v.digits.size() || carry; ++j) {
          long long cur = res.digits[i + j] +
                          (long long)digits[i] *
                              (j < (int)v.digits.size() ? v.digits[j] : 0) +
                          carry;
          carry = (int)(cur / BASE);
  ...
  bigint operator*(const bigint &v) const {
    if (digits.size() * v.digits.size() <= SIMPLE_MULT_BORDER)
      return mul_simple(v);
    if (digits.size() < FFT_BORDER || v.digits.size() < FFT_BORDER)
      return mul_fft(v);
  ...
  bigint mul_fft(const bigint &v) const {
    bigint res;
    res.sign = sign * v.sign;
    multiply_fft(convert_base(digits, BASE_DIGITS, 3),
                 convert_base(v.digits, BASE_DIGITS, 3), res.digits);
  ...
  inline bool isNegative() const { return !(isZero()) && (sign == -1); }

  inline bool isPositive() const { return !(isZero()) && (sign == 1); }
  ...
  friend bigint sqrt(const bigint &a1) {
    ...
    int firstDigit =
        (int)sqrt((double)a.digits[n - 1] * BASE + a.digits[n - 2]);
    ...
    for (int j = n / 2 - 1; j >= 0; j--) {
      for (;; --q) {
        bigint r1 =
            (r - (res * 2 * bigint(BASE) + q) * q) * bigint(BASE) *
                bigint(BASE) +
            (j > 0 ? (long long)a.digits[2 * j - 1] * BASE + a.digits[2 * j - 2]
                   : 0);
        if (r1 >= 0) {
          r = r1;
          break;
      ...
      if (j > 0) {
        int d1 = res.digits.size() + 2 < r.digits.size()
                     ? r.digits[res.digits.size() + 2]
                     : 0;
        int d2 = res.digits.size() + 1 < r.digits.size()
                     ? r.digits[res.digits.size() + 1]
                     : 0;
        int d3 = res.digits.size() < r.digits.size()
                     ? r.digits[res.digits.size()]
                     : 0;
...
std::pair<double, int> aproximation(const bigint &a) {
  if (a.isZero()) {
    return std::make_pair((double)0, int(0));
  }
  double mantis;
  int exponent;
  if (a.digits.size() == 1) {
    ...
    mantis =
        double(a.digits.back()) * 1000000000.0 + a.digits[a.digits.size() - 2];
